...
def test_release_artifact_smoke_accepts_expected_version(tmp_path: Path):
    ...
    def runner(*args, **kwargs):
        return subprocess.CompletedProcess(args[0], 0, stdout="Bookmark Organizer Pro v6.8.5\n", stderr="")

    result = smoke.smoke_artifact(artifact, expected_version="6.8.5", runner=runner)

    assert result.returncode == 0
    assert result.stdout == "Bookmark Organizer Pro v6.8.5"
...
def test_release_artifact_smoke_rejects_wrong_version(tmp_path: Path):
    ...
    try:
        smoke.smoke_artifact(artifact, expected_version="6.8.5", runner=runner)
