...
class TestOllamaSetupGuidance(unittest.TestCase):
    def test_manager_has_no_runtime_installer(self):
        from bookmark_organizer_pro.services import ollama_manager

        self.assertEqual(
            ollama_manager.OLLAMA_DOWNLOAD_URL,
            "https://ollama.com/download",
        )
        self.assertFalse(hasattr(ollama_manager.OllamaManager, "install"))

        source = Path(ollama_manager.__file__).read_text(encoding="utf-8")
        for forbidden in (
            "OllamaSetup.exe",
            "/VERYSILENT",
            "/NORESTART",
    ...
    def test_settings_opens_official_setup_page_without_invoking_installer(self):
        source = (
        ...
        self.assertIn("webbrowser.open(OLLAMA_DOWNLOAD_URL)", source)
        self.assertIn('text=_("Open Ollama Setup")', source)
        self.assertNotIn("OllamaManager().install(", source)
        self.assertNotIn("Install verified Ollama?", source)

    def test_refresh_reads_tk_state_before_starting_its_worker(self):
        source = (
            Path(__file__).resolve().parents[1]
            / "bookmark_organizer_pro"
            / "app_mixins"
        ...
        self.assertIn("mgr = OllamaManager(ollama_url)", source)
        self.assertNotIn("OllamaManager(ollama_url_var.get()", source)
        self.assertIn("before=reliability_separator", source)
