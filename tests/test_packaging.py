...
class TestNuitkaBuildHelper(unittest.TestCase):
    def test_command_includes_tk_assets_and_version_metadata(self):
        ...
        self.assertIn("--file-version=6.6.14.0", command)
        self.assertIn("--product-version=6.6.14.0", command)
    ...
    def test_command_accepts_custom_jobs(self):
        ...
        command = module.build_command(jobs=2, version="6.6.14", root=ROOT)
    ...
    def test_smoke_target_uses_console_entrypoint(self):
        ...
        command = module.build_command(target="smoke", version="6.6.14", root=ROOT)
