...
- [ ] P0: R-174: Move the dependency preflight out of package import
  Why: `preflight_or_exit()` raises `SystemExit(2)` inside the package `__init__`, so a missing manifest import kills any importer before it reaches an entry point. On 2026-09-04 `py -3.13 -m pytest --collect-only -q` ended in `INTERNALERROR ... SystemExit: 2` with "no tests collected" and exit 3, which means the whole test gate is silently unavailable on this machine's default interpreter.
  Evidence: `bookmark_organizer_pro/__init__.py:16`; `bootstrap_dependencies.py:168`; regression introduced by commit `8b8dd41`; reproduced locally 2026-09-04 on Python 3.13.15 with `tksheet` and `lz4` absent
  Touches: `bookmark_organizer_pro/__init__.py`, `bootstrap_dependencies.py`, `main.py`, `bookmark_organizer_pro/cli.py`, `bookmark_organizer_pro/mcp_server.py`, `bookmark_organizer_pro/desktop_bootstrap.py`, `tests/test_packaging.py`
  Acceptance: Importing `bookmark_organizer_pro` never raises `SystemExit`; the preflight runs at each documented entry point (desktop launcher, `cli.main`, `mcp_server.main`, frozen executable) and still names every missing import plus the interpreter-matched repair command; one test asserts `importlib.import_module("bookmark_organizer_pro")` succeeds in a clean subprocess with a manifest entry deliberately absent, and a second asserts each entry point still exits 2 with the complete message in that same environment; `pytest --collect-only` reports the full test count on an interpreter missing an optional manifest import.
  Complexity: S

- [ ] P0: R-175: Disclose or remove the near-duplicate scan cap
  Why: Passes 2 and 3 of hybrid duplicate detection examine only the first 5,000 non-exact-duplicate records and the report has no field to say so, so on a large library the desktop and CLI both announce that no near-duplicates were found after looking at a small fraction of it. That is a wrong answer presented as a clean result.
  Evidence: `bookmark_organizer_pro/services/dup_hybrid.py:100` `MAX_PAIRWISE = 5000`, sliced at `:122` and `:154`; `DuplicateReport` at `:90-92` carries only `groups` and `method_counts`; both callers pass the whole library, GUI at `bookmark_organizer_pro/app_mixins/tools.py:1046-1049` and CLI at `bookmark_organizer_pro/cli.py:1562-1564`; mass-duplication as a real user failure at https://news.ycombinator.com/item?id=41156568
  Touches: `bookmark_organizer_pro/services/dup_hybrid.py`, `bookmark_organizer_pro/app_mixins/tools.py`, `bookmark_organizer_pro/cli.py`, `bookmark_organizer_pro/ui/cleanup_review.py`, locale strings, `tests/test_services.py`
  Acceptance: `DuplicateReport` carries examined count, total count, and a truncated flag; every surface that renders a duplicate result states when the scan was partial and which records were skipped; the empty-result copy distinguishes "no near-duplicates in the records examined" from "no near-duplicates"; a scoped scan over an explicit selection or category runs without truncation so a user can complete the work in batches; `--json` output exposes the same fields; a test with a fixture above the cap asserts the truncation flag is set and that the flag is proved by lowering the cap and watching an untruncated fixture report it.
  Complexity: M
...
- [ ] P1: R-178: Make the benchmark thresholds scale with collection size
  Why: The gate holds every collection size to the same absolute millisecond ceiling, so it is a fixed budget rather than a complexity check. An O(n squared) rewrite that stays under 750 ms at 5,000 records passes it unchanged, which is why none of the scale defects in this pass were caught by a green benchmark run.
  Evidence: `benchmarks/bench_core.py:337-343` computes `scale = max(1.0, size / largest_size)`, which is exactly 1.0 for every size at or below the largest; `DEFAULT_SIZES` at `:32` is `(100, 1_000, 5_000)`; the measured cases at `:35-44` include no widget path and not `dup_hybrid.detect`
  Touches: `benchmarks/bench_core.py`, `tests/test_benchmarks.py`, `scripts/` benchmark invocation, README benchmark documentation
  Acceptance: Each case declares an expected growth class and the threshold is derived from the smallest tier's measured time scaled by that class, so a case that grows faster than declared fails even when it stays under a wall-clock ceiling; the size tiers extend above the current 5,000 once the open question about supported library size is settled; a `dup_hybrid.detect` case and a table-population case are added; the gate is proved by making one measured operation quadratic and watching only that case fail while the others stay green.
  Complexity: M
...
- [ ] P1: R-182: Capture crashes once the UI is running
  Why: Startup is guarded but `mainloop()` is not. There is no Tk exception callback, no `sys.excepthook`, no `threading.excepthook`, and no `faulthandler` anywhere, so once the window is up an unhandled exception goes to a stderr that a windowed or frozen build discards. Worker threads are guarded inconsistently, and the support bundle samples only the tail of a log a crash can rotate away.
  Evidence: no `report_callback_exception`, `sys.excepthook`, `threading.excepthook`, or `faulthandler` call exists in application code, `main.py`, or the packaging scripts as of 2026-09-04, the only tree matches being vendored `build/release-venv/` site-packages and an uncommitted visual-smoke addition; `main.py:110-150` guards startup only; unguarded workers at `bookmark_organizer_pro/app_mixins/ai_settings.py:295` and `bookmark_organizer_pro/app_mixins/selection.py:269-294` against a guarded one at `bookmark_organizer_pro/ui/live_workflow.py:205-215`; `bookmark_organizer_pro/logging_config.py:41-56` rotates at 5 MB with 3 backups; the support bundle samples the last 250 to 1000 lines from `bookmark_organizer_pro/services/local_state.py:491,533`
...
- [ ] P1: R-183: Make the MCP transport fallback loud and tested
  Why: The FastMCP builder swallows every construction failure and returns `None`, and the server then logs the fallback at info level and starts anyway without auto-schema, ToolAnnotations, or cache hints. A renamed symbol or an import error degrades the server silently, which matters more now that FastMCP 4 has renamed and removed APIs.
  Evidence: `bookmark_organizer_pro/mcp_server.py:1963` `getattr(fastmcp_mod, "FastMCP", None)`, `:1972` bare `except Exception: return None`, `:2286` and `:2301` info-level fallback; removals and renames documented at https://gofastmcp.com/getting-started/upgrading/from-fastmcp-3
  Touches: `bookmark_organizer_pro/mcp_server.py`, MCP diagnostics output, `bookmark_organizer_pro/cli.py` MCP status command, `tests/test_mcp_tools.py`
  Acceptance: A fastmcp module that is installed but unusable produces a warning naming the exact reason (missing attribute, constructor error, version) and a report of which capabilities the fallback loses; a completely absent fastmcp still falls back quietly because that case is expected; tests inject a double whose `FastMCP` raises and a double with no `FastMCP` attribute and assert both the warning text and the capability report; the CLI status command reports which transport is active.
  Complexity: S
...
- [ ] P1: R-186: Ship the message catalog directory in wheel and frozen builds
  Why: Catalog lookup resolves beside the package in a source checkout only, and no packaging path includes `locale/`, so a translation could never load from an installed wheel or a release executable. This is a second, unrecorded blocker sitting behind the parked first-translation task.
  Evidence: `bookmark_organizer_pro/i18n.py:33` resolves `_LOCALE_DIR` to `Path(__file__).resolve().parent.parent / "locale"`; `pyproject.toml` package-data ships only `bookmark_organizer_pro = ["**/*.json"]`; no `locale` reference in `packaging/bookmark_organizer.spec`, `packaging/nuitka_build.py`, or `scripts/build_release.py`
  Touches: `bookmark_organizer_pro/i18n.py`, `pyproject.toml`, `packaging/bookmark_organizer.spec`, `packaging/nuitka_build.py`, `scripts/build_release.py`, `packaging/product_claims.json`, `tests/test_packaging.py`, `tests/test_i18n.py`, `Roadmap_Blocked.md`
  Acceptance: Catalog lookup resolves through one helper covering source checkout, installed wheel, PyInstaller `_MEIPASS`, and Nuitka data roots; a compiled fixture catalog for one locale is installed by all three build paths and a test asserts a wrapped string returns its translated form from each root; the POT freshness gate is unchanged; the blocked first-translation entry records that its packaging half is cleared.
  Complexity: S
...
- [ ] P1: R-188: Report import completeness against the source
  Why: The strongest unmet ask found anywhere in the community evidence is simply knowing whether an import finished: how many records the file contained, how many landed, and what happened to the difference. The migration path already computes fidelity counts, but ordinary file and browser imports do not surface a reconciliation, and large imports are exactly where users report silent partial loss.
  Evidence: unanswered question at https://news.ycombinator.com/item?id=44597668 ("how many articles should have been imported ... to make sure it's complete"); large-collection imports "partially lose some data or fail completely" at https://news.ycombinator.com/item?id=41814394; migration preflight already produces counts in `bookmark_organizer_pro/services/migration.py`; ordinary importers return records without a reconciliation in `bookmark_organizer_pro/importers.py` and `bookmark_organizer_pro/importers_extra.py`; the Import Center presents source-specific guidance in `bookmark_organizer_pro/ui/import_center.py`
  Touches: `bookmark_organizer_pro/importers.py`, `bookmark_organizer_pro/importers_extra.py`, `bookmark_organizer_pro/services/batch_import.py`, `bookmark_organizer_pro/ui/import_center.py`, `bookmark_organizer_pro/cli.py`, locale strings, `tests/test_cli.py`
  Acceptance: Every importer returns a reconciliation carrying records found in the source, imported, skipped as duplicates, and rejected with a reason, and the totals add up to the found count or the import reports an accounting error rather than a success; the Import Center and the CLI both show it, and `--json` exposes the same structure; rejected rows are written to a reviewable list the user can open, not only counted; a malformed or truncated source file is reported as such instead of importing a partial file quietly; a test imports a fixture with duplicates, malformed rows, and unsupported entries and asserts every category is counted and the arithmetic balances.
  Complexity: M
