...
- **CLI:** 56 subcommands, 444 collected tests in the current suite
...
### Cycle Note — v6.6.24 (2026-06-06)

R-41 remains in progress. Successful updater target staging now writes
`updates/staged_update.json` with target metadata, staged paths, channel, and
timestamp. `UpdateManager.staged_update()` and `updates staged` validate the
manifest and report whether target files are still present under
...
### Cycle Note — v6.6.26 (2026-06-06)

R-41 remains in progress. BOP now exposes staged updater cleanup through
`UpdateManager.clear_staged_update()` and `updates clean-staged`, removing only
the staged manifest and cached staged targets under `updates/targets/`. Trusted
metadata and application files are untouched. Details:
`docs/audit/2026-06-06-v6.6.26-updater-staged-cleanup-audit.md`.

### Cycle Note — v6.6.27 (2026-06-06)

R-41 remains in progress. BOP now exposes non-mutating apply planning through
`UpdateManager.build_apply_plan()` and `updates plan`, reporting staged targets,
install path, rollback path, planned actions, and blockers without creating,
extracting, installing, executing, or replacing files. Details:
`docs/audit/2026-06-06-v6.6.27-updater-apply-plan-audit.md`.

### Cycle Note — v6.6.28 (2026-06-06)

R-15 remains in progress. `chat_with_collection_stream` now publishes a stable
stream contract version and event type list in both final payloads and MCP tool
metadata. Raw and FastMCP catalog tests plus the FastMCP client smoke now verify
...
### Cycle Note — v6.6.30 (2026-06-06)

R-15/R-58 transport hardening advanced. Streamable HTTP mirrored headers are
now optional for standard MCP clients while mismatched provided headers still
return validation errors. This keeps router/observability metadata useful
without blocking clients that only send JSON-RPC bodies. Details:
`docs/audit/2026-06-06-v6.6.30-mcp-http-headers-audit.md`.
...
| ⏸️ R-02 | **Web client (FastAPI + HTMX)** — moved to `Roadmap_Blocked.md`. XL effort requiring full web architecture. | Blocked | XL | [S-3][S-5][S-6][S-8][S-12] |
| ⏸️ R-03 | **Mobile PWA share-intent** — moved to `Roadmap_Blocked.md`. Depends on R-02 (web client). | Blocked | M | [S-8][S-12] |
...
| ⏸️ R-41 | **tufup auto-update** — non-mutating infrastructure complete (policy, check, download, staging, preflight, cleanup, planning) through v6.6.27. Apply stage moved to `Roadmap_Blocked.md` — requires install isolation, rollback, and user confirmation design. | Blocked | M | [S-28][S-88] |
...
| R-15G | Streamable HTTP optional mirrored-header compatibility | v6.6.30 |
| R-15 | Native streaming for all 6 providers (Anthropic + Google added) | v6.8.1 |
...
| R-41G | Staged updater artifact cleanup | v6.6.26 |
| R-41H | Non-mutating updater apply and rollback plan | v6.6.27 |
...
### Later — v7.x+ (0 active/open; R-15/R-17/R-18/R-21/R-22/R-26/R-50 shipped; R-41/R-02/R-03 blocked — see Roadmap_Blocked.md)

All Later-tier items are either shipped or moved to `Roadmap_Blocked.md`.
