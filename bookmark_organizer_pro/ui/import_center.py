...
def build_import_sources(detected_browsers: Iterable[str] = ()) -> List[ImportSource]:
    ...
    sources = [
        ...
        ImportSource(
            key="omnivore",
            title=_("Omnivore export"),
            description="Import an Omnivore export archive after the service shut down.",
            accepted_formats="Export .zip, unpacked folder, or a metadata_*.json file",
            privacy_note="The archive is read locally; no Omnivore connection is attempted.",
            duplicate_policy="Repeated URLs across metadata batches are collapsed, then existing library URLs are skipped.",
            import_summary="Result shows added bookmarks, duplicates, and unusable entries.",
            next_action="Review imported labels as tags and check the Read Later queue.",
            action_label="Choose Omnivore Export",
            action_kind="service",
            action_arg="omnivore",
        ),
        ImportSource(
            key="csv",
            title=_("CSV export"),
            description="Import any CSV with a URL column, including Markwise and start.me.",
            accepted_formats="CSV with a URL column (.csv)",
            privacy_note="The file is parsed locally; no service connection is made.",
            duplicate_policy="Existing normalized URLs are skipped.",
            import_summary="Result shows added bookmarks, duplicates, and rows without a URL.",
            next_action="Review mapped categories, then run tag cleanup if needed.",
            action_label="Choose CSV File",
            action_kind="service",
            action_arg="csv",
        ),
        ImportSource(
